...
func ParseFromNibbles(nibbles []Nibble) (Instruction, error) {
	...
	case 0x1:
		return Parse(fmt.Sprintf("mov [%x] -> r%x", CombineNibblesToByte(nibbles[2], nibbles[3]), nibbles[1]))
	case 0x2:
		return Parse(fmt.Sprintf("mov %x -> r%x", CombineNibblesToByte(nibbles[2], nibbles[3]), nibbles[1]))
	case 0x3:
		return Parse(fmt.Sprintf("mov r%x -> [%x]", nibbles[2], CombineNibblesToByte(nibbles[3], nibbles[1])))
	case 0x4:
		return Parse(fmt.Sprintf("mov r%x -> r%x", nibbles[2], nibbles[3]))
	case 0x5:
		return Parse(fmt.Sprintf("addi r%x, r%x -> r%x", nibbles[2], nibbles[3], nibbles[1]))
	case 0x6:
		return Parse(fmt.Sprintf("addf r%x, r%x -> r%x", nibbles[2], nibbles[3], nibbles[1]))
	case 0x7:
		return Parse(fmt.Sprintf("or r%x, r%x -> r%x", nibbles[2], nibbles[3], nibbles[1]))
	case 0x8:
		return Parse(fmt.Sprintf("and r%x, r%x -> r%x", nibbles[2], nibbles[3], nibbles[1]))
	case 0x9:
		return Parse(fmt.Sprintf("xor r%x, r%x -> r%x", nibbles[2], nibbles[3], nibbles[1]))
	case 0xa:
		return Parse(fmt.Sprintf("rot r%x, %x", nibbles[2], CombineNibblesToByte(nibbles[3], nibbles[1])))
	case 0xb:
		return Parse(fmt.Sprintf("jmpeq %x, r%x", CombineNibblesToByte(nibbles[2], nibbles[3]), nibbles[1]))
	case 0xc:
...
func (instr *Instruction) parseExecuteMethodsAndBytes() error {
	...
	case "mov:[->r":
		...
		nibbles = []Nibble{0x01, Nibble(instr.RightOperands[0].Value), splitOp[0], splitOp[1]}
	case "mov:v->r":
		execMethod = func(vm *VM) { vm.doMoveValToReg(instr.LeftOperands[0].Value, instr.RightOperands[0].Value) }
		var splitOp = SplitByteToNibbles(instr.LeftOperands[0].Value)
		nibbles = []Nibble{0x02, Nibble(instr.RightOperands[0].Value), splitOp[0], splitOp[1]}
	case "mov:r->[":
	...
	case "mov:r->r":
		...
		nibbles = []Nibble{0x04, 0x00, Nibble(instr.RightOperands[0].Value), Nibble(instr.LeftOperands[0].Value)}
	case "addi:rr->r":
		...
		nibbles = []Nibble{0x05, Nibble(instr.RightOperands[0].Value), Nibble(instr.LeftOperands[0].Value), Nibble(instr.LeftOperands[1].Value)}
	case "addf:rr->r":
		...
		nibbles = []Nibble{0x06, Nibble(instr.RightOperands[0].Value), Nibble(instr.LeftOperands[0].Value), Nibble(instr.LeftOperands[1].Value)}
	case "or:rr->r":
		...
		nibbles = []Nibble{0x07, Nibble(instr.RightOperands[0].Value), Nibble(instr.LeftOperands[0].Value), Nibble(instr.LeftOperands[1].Value)}
	case "and:rr->r":
		...
		nibbles = []Nibble{0x08, Nibble(instr.RightOperands[0].Value), Nibble(instr.LeftOperands[0].Value), Nibble(instr.LeftOperands[1].Value)}
	case "xor:rr->r":
		...
		nibbles = []Nibble{0x09, Nibble(instr.RightOperands[0].Value), Nibble(instr.LeftOperands[0].Value), Nibble(instr.LeftOperands[1].Value)}
	case "rot:rv":
	...
	case "jmpeq:vr":
		...
		var splitOp = SplitByteToNibbles(instr.LeftOperands[0].Value)
		nibbles = []Nibble{0x0b, Nibble(instr.LeftOperands[1].Value), splitOp[0], splitOp[1]}
	case "halt":
